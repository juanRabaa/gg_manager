...
var panelProductos = angular.module('panelProductos', ['ngRoute', 'ui.materialize', "angucomplete-alt", 'ui.sortable']);
...
panelProductos.factory('pagesFactory', function($http) {
    var pagesFactory = {
        getPagesList: function(){
            return $http.get(templateUrl + '/wp-json/wp/v2/pages?orderby=title');
        },
    };
...
panelProductos.directive('rbAttr', function (){
  return {
     restrict: 'A',
     scope: {
        'rbAttr' : '='
     ...
     link: function(scope, element, attrs) {
         scope.stablishAttributes = function (){
             for (var attribute in scope.rbAttr) {
                 if (scope.rbAttr[attribute])
                     element.attr(attribute, '');
                 else
         ...
         scope.$watch('rbAttr', function() {
             scope.stablishAttributes();
         }, true);
     }
  };
});
